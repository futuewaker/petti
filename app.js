...
(function () {
  ...
  const MOCK_QUESTIONS = [
    {
      ...
      options: [
        ...
        { label: '关灯拉窗帘，一个人和我的宠物',     score: -1 }
      ]
    }
  ];
  const MOCK_PETS = [
    {
      id: 'fox', name: '狐狸', name_title: '机灵鬼的', image: 'images/fox.png',
      slogan: '看似高冷，心里有火',
  ...
  async function init() {
    const [questions, pets] = await Promise.all([
      ...
      loadJSON('data/pets.json',      MOCK_PETS)
    ]);
  ...
  async function loadJSON(path, fallback) {
    try {
      ...
      console.warn('[PETTI] fallback to mock for', path, err.message);
      return fallback;
    }
  }
  ...
  async function embedImagesAsDataUrl(root) {
    ...
    await Promise.all(Array.from(imgs).map(async function (img) {
      ...
      try {
        ...
        console.warn('[PETTI] embed img fail, keep original', src, e);
      }
    }));
  }
  ...
  function snapshotCanvases(root) {
    ...
    root.querySelectorAll('canvas').forEach(function (canvas) {
      try {
        ...
        console.warn('[PETTI] canvas snapshot failed', e);
      }
    });
  ...
  async function saveLongScreenshot() {
    ...
    try {
      ...
      const filename = 'petti-' + pet.id + '-' + Date.now() + '.png';
      ...
      try {
        ...
        else console.warn('[PETTI] share API failed, falling back', e);
      }
      ...
      console.error('[PETTI] long screenshot failed', err);
      alert('截图生成失败,请稍后重试~');
    } finally {
